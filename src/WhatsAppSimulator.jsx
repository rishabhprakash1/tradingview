...
function TradeBubble({ client, onAccept, onReject }) {
  ...
  return (
    <div className="flex flex-col w-full px-3 pt-3 gap-3 mb-4">
      {/* ── Bubble 1: Trade Request (from Krijuna Research) ── */}
      ...
      {status !== TRADE_STATUSES.WAITING_FOR_USER && (
        <div className="self-end bg-[#D9FDD3] rounded-tl-2xl rounded-br-2xl rounded-bl-2xl shadow-sm px-3 py-2 max-w-[80%] flex flex-col items-end">
          <span className="text-sm text-[#111B21]">Trade Accepted</span>
          <div className="flex items-center gap-1 mt-1">
            <span className="text-[10px] text-[#667781]">{time}</span>
            <svg className="h-3.5 w-3.5 text-[#53bdeb]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* ── Bubble 2: Broker Notified ── */}
      {(status === TRADE_STATUSES.USER_CONFIRMED || status === TRADE_STATUSES.EXECUTED) && (
        <div className="self-start bg-white rounded-tr-2xl rounded-br-2xl rounded-bl-2xl shadow-sm px-3 py-2 max-w-[85%] border-l-4 border-green-500 mt-1">
          <p className="text-[11px] font-semibold text-green-600 mb-0.5">System Alert</p>
          <span className="text-sm text-[#111B21]">Broker has been notified ✅</span>
          <p className="text-right text-[10px] text-[#667781] mt-1">{time}</p>
        </div>
      )}

      {/* ── Bubble 3: Trade Executed ── */}
      {status === TRADE_STATUSES.EXECUTED && (
        <div className="self-start bg-white rounded-tr-2xl rounded-br-2xl rounded-bl-2xl shadow-sm px-3 py-2 max-w-[85%] border-l-4 border-blue-500 mt-1">
          <p className="text-[11px] font-semibold text-blue-600 mb-0.5">System Alert</p>
          <span className="text-sm text-[#111B21]">Trade Executed Successfully 🚀</span>
          <p className="text-right text-[10px] text-[#667781] mt-1">{time}</p>
        </div>
      )}
    </div>
